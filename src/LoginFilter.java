...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if(this.isEmployeeDashboard(httpRequest.getRequestURI())){
        	if(httpRequest.getSession().getAttribute("emp") == null){
        		httpResponse.sendRedirect("employeelogin.html");
        	}else {
        		chain.doFilter(request, response);
        		//httpResponse.sendRedirect("dashbaord.html");
        		return;
        	}
        }

        // Check if the URL is allowed to be accessed without log in
        else if (this.isUrlAllowedWithoutLogin(httpRequest.getRequestURI())) {
    ...
    private boolean isUrlAllowedWithoutLogin(String requestURI) {
        ...
        return requestURI.endsWith("login.html") || requestURI.endsWith("login.js")
                || requestURI.endsWith("api/login") || requestURI.contains("api") ||requestURI.contains("dist");
    }

    private boolean isEmployeeDashboard(String requestURL) {
    	requestURL = requestURL.toLowerCase();
    	return requestURL.contains("dashboard");
    }
    /**
